...
fill_extra_data(Conn *c)//解析客户端发来的任务数据，存储在c->in_job的body数据字段
{
    ...
    cmd_bytes = extra_bytes - job_data_bytes;//如果extra_bytes>job_data_bytes  cmd_bytes就是下个命令的
...
maybe_enqueue_incoming_job(Conn *c) //校验job数据是否读取完毕，完了则入tube的队列
{
    ...
    c->state = STATE_WANTDATA;//任务数据没有读取完毕，则设置客户端conn状态未等待接收数据STATE_WANTDATA
}
...
dispatch_cmd(Conn *c)
{
    ...
    case OP_LIST_TUBES_WATCHED:
        ...
        break;
    case OP_USE://使用指定的tube 如果没有这个tube，那么就创建这个tube，放入tube ms      use和using命令，影响的是put操作。而且只能有一个tube被use。
...
conn_data(Conn *c)//客户端数据交互（根据客户端状态不同执行不同的读写操作）
{
    ...
    case STATE_SENDWORD:
        ...
        if (c->reply_sent == c->reply_len) return reset_conn(c);//如果job的数据已经发完，则重置客户端rw，关心可读事件；否则继续待发送job
